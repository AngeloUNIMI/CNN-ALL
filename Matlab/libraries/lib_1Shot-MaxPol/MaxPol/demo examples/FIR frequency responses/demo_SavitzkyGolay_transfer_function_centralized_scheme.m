...
switch experiment_number
    case 1
        ...
        tap_length = tap_length_polynmial*ones(1,tap_length_polynmial+1);
        possible_cutoffs = [0: 2: 2*tap_length_polynmial];
        side_shifts = [0];
        n_ords = [0, 1, 2, 3, 4];
...
for n_ord = n_ords
    ...
    for s = side_shifts
        ...
        for p = possible_cutoffs(possible_cutoffs>=n_ord)
            ...
            %%  filter response
            [c{iteration_n, iteration}] = derivcent_SavitzkyGolay(l, p, s, n_ord, sym_flag);
