function [D, D_forward] = derivmtx_Fornberg(l, n, N, nod, sparse_flag, sym_flag)

%DERIVMTX_FORNBERG Fornberg Derivative square matrix
%
%   [D, D_forward] = derivmtx_Fornberg(l, n, N, nod, sparse_flag, sym_flag)
%
%   returns NxN derivative matrix incorporated by Fornberg FIR derivative
%   kernels at every row of the matrix. d^nf = D*f estimates n'th order
%   derivative of discrete vector-vlaued function 'f'.
%
%   Input(s):
%   'l'             polynomial degree
...
switch nod
    case 'staggered'
        ...
        [c] = derivstag_Fornberg(l, 0, n, sym_flag);
        row = [c, zero_stacks];
        ...
        block_left = [];
        for s = 1: l
            [c] = derivstag_Fornberg(l, s, n, sym_flag);
    ...
    case 'centralized'
        ...
        [c] = derivcent_Fornberg(l, 0, n, sym_flag);
        R = [c, zero_stacks];
        ...
        block_left = [];
        for s = 1: l
            [c] = derivcent_Fornberg(l, s, n, sym_flag);
            block_left = [[c, zero_stacks]; block_left];
        end
        %
        block_right = [];
